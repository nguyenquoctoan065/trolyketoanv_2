...
// Tăng thời gian chạy serverless function trên Vercel lên mức tối đa để không bị quá hạn (timeout) khi AI phân tích
export const maxDuration = 60;

// Cấu hình multer lưu trữ trong bộ nhớ tạm
const upload = multer({ storage: multer.memoryStorage() });

// Vercel yêu cầu tắt bodyParser mặc định để multer có thể đọc FormData
export const config = {
  api: {
    bodyParser: false,
  },
...
// Hàm hỗ trợ chạy middleware multer trong serverless function Vercel
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({ error: "Thiếu API Key của Gemini. Vui lòng thêm GEMINI_API_KEY vào Environment Variables (cài đặt biến môi trường) trên Vercel." });
    }

    // Chạy multer để đọc body form-data
    ...
    const base64Data = (req as any).file.buffer.toString("base64");
...
- Các số tiền không nên có dấu phẩy hay ký tự tiền tệ, chỉ để số (Number).`;
    ...
    if (outputText) {
      ...
      res.status(500).json({ error: "Không thể trích xuất dữ liệu từ ảnh." });
    }
    ...
    if (error && (error.status === 429 || (error.message && (error.message.includes("429") || error.message.includes("Quota"))))) {
      errorMessage = "Hệ thống Google báo lỗi: Đã vượt quá giới hạn lượt dùng API Gemini miễn phí (Quota exceeded - 429). Do tài khoản Google của bạn đã hết hạn mức (20 request / ngày ...), vui lòng đổi API Key khác hoặc nâng cấp gói API.";
