  ...
  const steps = [
    {
      ...
      icon: (
        <Upload
          className="w-14 h-14 text-primary-500 mb-6 drop-shadow-sm"
          strokeWidth={1.5}
        />
      ),
    },
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden relative border border-white/20 animate-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="px-8 flex items-center justify-center gap-2.5 mb-8">
          {steps.map((_, i) => (
            <div
              key={i}
              className={`h-2 rounded-full transition-all duration-300 ${i === step ? "w-8 bg-primary-500 shadow-sm shadow-primary-500/30" : "w-2 bg-gray-200"}`}
            />
          ))}
        </div>
  ...
  return (
    <header className="flex items-center justify-between py-4 px-4 md:px-8 bg-white/60 backdrop-blur-xl border-b border-gray-100 z-10 sticky top-0">
      <div className="flex items-center gap-3">
        <button
          onClick={onMenuClick}
          className="p-2 -ml-2 rounded-xl text-gray-500 hover:bg-gray-100 lg:hidden"
        >
          <Menu size={24} />
        </button>
        <h2 className="text-xl md:text-2xl font-display font-semibold text-gray-800">
          {titles[activeTab] || "AccoBot"}
        </h2>
      </div>
    </header>
  );
...
function AppContent() {
  const [activeTab, setActiveTab] = useState<
    "upload" | "review" | "table" | "dashboard" | "help"
  >("upload");
  ...
  const handleTabChange = (
    tab: "upload" | "review" | "table" | "dashboard" | "help",
  ) => {
    setActiveTab(tab);
  ...
  return (
    <div className="min-h-screen flex bg-[#f8fafc] font-sans">
      ...
      {/* Sidebar Navigation */}
      <nav
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-gray-900 flex-shrink-0 flex flex-col text-gray-300 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="p-8 pb-6 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-primary-300/80 font-medium tracking-wide">
                TRỢ LÝ KẾ TOÁN AI
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-1.5 px-4 pb-6 overflow-y-auto flex-1 mt-6">
          <p className="px-4 text-xs font-semibold text-gray-500 tracking-wider mb-2 mt-4 uppercase">
            Chức năng chính
          </p>
          <NavItem
            icon={<Upload size={18} />}
          ...
          <NavItem
            ...
            label="Kho lưu trữ HĐ"
            isActive={activeTab === "table"}
            onClick={() => handleTabChange("table")}
          />

          <p className="px-4 text-xs font-semibold text-gray-500 tracking-wider mb-2 mt-6 uppercase">
            Báo cáo & Phân tích
          </p>
          <NavItem
            icon={<BarChart3 size={18} />}
            label="Tổng quan Budget"
...
function NavItem({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <span
          className={`${isActive ? "text-primary-500" : "group-hover:text-gray-300"}`}
        >
          {icon}
        </span>
        {label}
      ...
      {badgeCount !== undefined && badgeCount > 0 && (
        <span
          className={`px-2 py-0.5 rounded-md text-xs font-bold ${isActive ? "bg-primary-500 text-white shadow-sm shadow-primary-500/30" : "bg-gray-800 text-gray-300"}`}
        >
          {badgeCount}
        </span>
      )}
