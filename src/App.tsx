...
import { Upload, FileText, CheckCircle, BarChart3, ChevronRight, X, HelpCircle, UserX } from 'lucide-react';
...
function TopHeader({ activeTab }: { activeTab: string }) {
  ...
  return (
    <header className="flex items-center justify-between py-4 px-8 bg-white/60 backdrop-blur-xl border-b border-gray-100 z-10 sticky top-0">
      <div>
         <h2 className="text-2xl font-display font-semibold text-gray-800">{titles[activeTab] || 'AccoBot'}</h2>
      </div>
    </header>
  )
}
...
function AppContent() {
  ...
  return (
    <div className="min-h-screen flex bg-[#f8fafc] font-sans">
      ...
      {/* Sidebar Navigation */}
      <nav className="w-72 bg-gray-900 flex-shrink-0 flex flex-col z-20 text-gray-300 transition-all">
        ...
        <div className="flex flex-col gap-1.5 px-4 pb-6 overflow-y-auto flex-1 mt-6">
          ...
          <NavItem 
            ...
            isActive={activeTab === 'upload'} onClick={() => setActiveTab('upload')} 
          />
          <NavItem 
            icon={<FileText size={18} />} label="Hóa đơn chờ duyệt" 
            badgeCount={pendingCount}
            isActive={activeTab === 'review'} onClick={() => setActiveTab('review')} 
          />
          <NavItem 
            icon={<CheckCircle size={18} />} label="Kho lưu trữ HĐ" 
            isActive={activeTab === 'table'} onClick={() => setActiveTab('table')} 
          />
          ...
          <div className="mt-auto pt-6 flex flex-col gap-1.5">
            <NavItem 
              ...
              isActive={activeTab === 'help'} onClick={() => setActiveTab('help')} 
            />
            <button
               onClick={() => {
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-[#fafafa] relative">
         ...
         <div className="flex-1 overflow-y-auto p-6 md:p-8 relative scroll-smooth">
           <div className="max-w-[1400px] mx-auto w-full relative z-10 transition-all duration-300">
             {activeTab === 'upload' && <UploadSection onComplete={() => setActiveTab('review')} />}
